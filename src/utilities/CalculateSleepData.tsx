...
export default function CalculateSleepData({
	...
	if (
		bedTimeHour ||
		(bedTimeHour === 0 && bedTimeMinute) ||
		(bedTimeMinute === 0 && wakeUpHour) ||
		(wakeUpHour === 0 && wakeUpMinute) ||
		wakeUpMinute === 0
	) {
	...
	// Calculator 2: User inputs only wake up time
		...
		wakeUpTimeInMinutes = (wakeUpHour ?? 0) * 60 + (wakeUpMinute ?? 0);
		bedTimeInMinutes = wakeUpTimeInMinutes - 7.5 * 60; // 7 hours 30 minutes earlier
	...
	const optimalBedTimeInMinutes = wakeUpTimeInMinutes - 450;
	const optimalBedTimeHour = Math.floor(optimalBedTimeInMinutes / 60);
	const optimalBedTimeMinute = optimalBedTimeInMinutes % 60;
